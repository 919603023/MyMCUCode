...
void *Qt_Server(void *arg);
void *GSM_To_QT(void *arg);
int main(int argc, char const *argv[])
{
    ...
    while(1)
    {
        ...
        //创建一个单独的线程 服务于客户端
        pthread_t tid,tid1;
        pthread_create(&tid,NULL,Qt_Server, &new_fd);
         pthread_create(&tid1,NULL,GSM_To_QT, &new_fd);
        pthread_detach(tid);
         pthread_detach(tid1);
    }
...
else if((strncmp(buf,"close",5) > 0) && (strncmp(buf,"open",4) > 0))
{
...
void *GSM_To_QT(void *arg)
{
    ...
    while (1)
    {
        msgrcv(ret_msg,&msg,sizeof(msg)-sizeof(long),50,0);
        printf("服务器发出：%s\n",msg.data);
...
void My_Kill(int sig) //ctrl+c退出，销毁消息队列
{
	printf("安全结束\n");
